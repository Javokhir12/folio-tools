...
/**
 * Create ticket the given component.
 *
 * for all jira projects associated with packages in
...
class TestFor {
  ...
  parseArgv()
  {
    return yargs(hideBin(process.argv))
      ...
      .option('c', {
        alias: 'components',
        describe: 'path to file listing components to create tests for',
        type: 'string',
      })

      .option('p', {
        alias: 'project',
        describe: 'github repository containing component',
        type: 'string',
      })
  ...
  async main()
  {
    ...
    try {
      ...
      // map dependencies:
      const components = fs.readFileSync(this.argv.components, { encoding: 'UTF-8'}).split(/\n/);
      ...
      .then(projects => {
        ...
        const pmap = {};
        projects.data.forEach(p => { pmap[p.name.trim()] = p; });

        if (pmap[this.argv.project] && this.matrix[this.argv.project]) {
          const repo = this.argv.project;

          this.eachPromise(components, c => {
            if (!c) return;

            this.teamForName(this.matrix[this.argv.project].team)
            .then(team => {
              // only assign the team if we received --team
              const t = this.argv.team ? team : null;
              const cc = [];
              if (this.argv.ccpo && this.matrix[repo].po) {
                cc.push(this.matrix[repo].po)
              }

              if (this.argv.cctl && this.matrix[repo].tl) {
                cc.push(this.matrix[repo].tl)
              }

              const file = c.split('/').pop();
              return this.createTicket({
                summary: `create Jest/RTL test for ${file}`,
                description: `*Summary*: create Jest/RTL test for ${c}`,
                project: pmap[repo],
                epic: this.argv.epic,
          ...
          console.warn(`could not find a jira project matching ${this.argv.project}`);
          console.log(pmap)
